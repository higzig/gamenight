...
import { activeIBetYouGroup, adjustBid, iBetYouSecondsRemaining, teamName } from './src/i-bet-you.js';
...
function renderRemoteIBetControl(){
  ...
  const memberOptions=group.members.map(m=>`<option value="${m.team_id}">${esc(m.name)}</option>`).join(''),bidder=group.challenged_bidder_team_id||group.current_bidder_team_id||group.members[0]?.team_id,challenger=group.challenger_team_id||group.members.find(m=>m.team_id!==bidder)?.team_id,bid=group.target_bid||group.current_bid||1;
  let controls='';
  if(['waiting','bidding'].includes(group.state))controls=`<div class="ibet-bidder-grid">${group.members.map(m=>`<button class="ibet-team-button ${m.team_id===group.current_bidder_team_id?'selected':''}" data-team="${m.team_id}">${esc(m.name)}</button>`).join('')}</div><div class="ibet-bid-control"><button class="ibet-number-button" id="ibetMinus">−</button><strong>${bid}</strong><button class="ibet-number-button" id="ibetPlus">+</button></div><label class="field">Challenger<select id="ibetChallenger">${group.members.filter(m=>m.team_id!==(group.current_bidder_team_id||bidder)).map(m=>`<option value="${m.team_id}">${esc(m.name)}</option>`).join('')}</select></label><button class="btn primary ibet-primary-action" id="ibetChallenge" ${!group.current_bidder_team_id?'disabled':''}>NAME THEM</button>`;
  ...
  document.querySelectorAll('.ibet-team-button').forEach(b=>b.onclick=()=>window.gameNightSupabaseActions.setIBetYouBid(group.id,b.dataset.team,bid));if($('#ibetMinus'))$('#ibetMinus').onclick=()=>window.gameNightSupabaseActions.setIBetYouBid(group.id,group.current_bidder_team_id||bidder,adjustBid(bid,-1));if($('#ibetPlus'))$('#ibetPlus').onclick=()=>window.gameNightSupabaseActions.setIBetYouBid(group.id,group.current_bidder_team_id||bidder,adjustBid(bid,1));if($('#ibetChallenge'))$('#ibetChallenge').onclick=()=>window.gameNightSupabaseActions.challengeIBetYou(group.id,$('#ibetChallenger').value);
