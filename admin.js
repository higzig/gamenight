...
function selectControlRound(id,{openLive=true}={}){const s=remoteSession();if(s?.event?.id)localStorage.setItem(CONTROL_ROUND_PREFIX+s.event.id,id);if(openLive){document.querySelector('[data-view="live"]')?.click()}else renderLiveControl()}
function roundNavigator(){const selected=selectedControlRound(),active=remoteSession()?.event?.active_round_id;return `<section class="round-navigator" aria-label="Round navigator"><h3>Rounds</h3><div>${controlRounds().map(r=>`<button class="round-tab ${r.id===selected?.id?'selected':''} ${r.id===active?'live':''}" data-control-round="${r.id}" ${r.configured?'':'disabled'} aria-pressed="${r.id===selected?.id}"><span>${r.position||'—'}</span>${esc(r.title||r.game_type||'Not configured')}${r.id===active?'<i>Live</i>':''}</button>`).join('')}</div></section>`}
function bindRoundNavigator(){document.querySelectorAll('[data-control-round]').forEach(button=>button.onclick=()=>selectControlRound(button.dataset.controlRound))}
function audienceSelector(event){return `<div class="audience-control"><span>Audience display</span><div>${audienceDisplayOptions(event.display_mode).map(mode=>`<button class="segment ${mode.selected?'selected':''}" data-display="${mode.value}" aria-pressed="${mode.selected}">${mode.label}</button>`).join('')}</div></div>`}
function bindAudienceSelector(){document.querySelectorAll('[data-display]').forEach(button=>button.onclick=()=>window.gameNightSupabaseActions.setDisplay(button.dataset.display))}
...
function renderRounds(){
  $('#roundsList').innerHTML=state.rounds.map((r,i)=>{const g=games[r.type]||games.future;return `<div class="round-card" data-id="${r.id}"><div class="round-num">${i+1}</div><div class="game-icon">${g.icon}</div><div class="round-main"><h3>${esc(r.title||g.name)}</h3><p>${esc(roundSummary(r))}</p></div><div class="round-actions"><span class="round-status ${g.ready?'':'todo'}">${g.ready?'Ready':'Needs setup'}</span>${g.ready?'<button class="mini-btn open-round">Open</button>':''}<button class="mini-btn move-up" aria-label="Move round up" ${i===0?'disabled':''}>↑</button><button class="mini-btn move-down" aria-label="Move round down" ${i===state.rounds.length-1?'disabled':''}>↓</button><button class="mini-btn edit-round">Edit</button></div></div>`}).join('');
  ...
  document.querySelectorAll('.open-round').forEach(b=>b.onclick=()=>{const round=state.rounds.find(x=>x.id===b.closest('.round-card').dataset.id),gameType=round?.type==='guessAge'?'guess_age':round?.type==='iBetYou'?'i_bet_you':null,target=controlRounds().find(x=>x.game_type===gameType);if(target)selectControlRound(target.id)});
}
...
function openDrawer(id){editingRoundId=id;const r=state.rounds.find(x=>x.id===id);$('#drawerTitle').textContent=r.title||games[r.type].name;renderDrawer(r);document.getElementById('drawerSync')?.remove();if(r.type==='guessAge'&&remoteSession()){const button=document.createElement('button');button.id='drawerSync';button.className='btn secondary';button.textContent='Sync Guess the Age';button.onclick=()=>$('#saveEvent').click();$('#doneRound').before(button)}$('#roundDrawer').classList.add('open');$('#drawerBackdrop').classList.add('open')}
...
function renderRemoteIBetSetup(){const event=remoteSession().event;$('#liveControl').innerHTML=`${roundNavigator()}<div class="host-display-row">${audienceSelector(event)}<button class="btn secondary" id="openAudience">Audience ↗</button></div><section class="control-empty"><h2>I Bet You</h2><p>Create groups and categories from the Teams currently joined. This does not begin bidding.</p><button class="btn primary" id="prepareIBet">Prepare I Bet You</button></section>`;bindRoundNavigator();bindAudienceSelector();$('#openAudience').onclick=()=>window.open(`audience.html?room=${encodeURIComponent(event.room_code)}`,'gameNightAudience');$('#prepareIBet').onclick=()=>window.gameNightSupabaseActions.setupIBetYou()}
function polishRemoteGuessControl(event,status,q,answered){const toolbar=$('#liveControl .live-toolbar');if(toolbar)toolbar.outerHTML=`${roundNavigator()}<div class="host-display-row">${audienceSelector(event)}<button class="btn secondary" id="openAudiencePolished">Audience ↗</button></div>`;bindRoundNavigator();bindAudienceSelector();if($('#openAudiencePolished'))$('#openAudiencePolished').onclick=()=>window.open(`audience.html?room=${encodeURIComponent(event.room_code)}`,'gameNightAudience');const main=$('#liveControl .live-main');main?.querySelector('.panel-heading .label')?.remove();const title=main?.querySelector('.panel-heading h2');if(title)title.textContent='Guess the Age';main?.querySelector('.live-meta')?.remove();const answerText=main?.querySelector('.question-preview p:last-child');if(answerText)answerText.innerHTML=`Answer: <strong>${q?ageOn(q.date_of_birth,event.event_date):'—'}</strong> · ${answered}/${remoteTeams().length} locked`;const action=guessPrimaryAction(status),start=$('#startQuestion'),next=$('#nextQuestion');if(start){start.hidden=action.id!=='startQuestion';start.textContent='Start question'}if(next){next.hidden=action.id!=='nextQuestion';next.className='btn primary';next.textContent='Next question'}const side=$('#liveControl .live-side');side?.querySelector('.panel-heading .label')?.remove();side?.querySelector('.panel-heading .pill')?.remove();const sideTitle=side?.querySelector('.panel-heading h2');if(sideTitle)sideTitle.textContent='Answers';side?.querySelector('#showLeaderboard')?.remove();const recovery=side?.querySelector('.recovery-actions');if(recovery)recovery.innerHTML=`<details class="more-menu"><summary>More</summary><button class="danger-link" id="restartRoundPolished">Restart this round</button><button id="newSessionPolished">Start new session</button></details>`;if($('#restartRoundPolished'))$('#restartRoundPolished').onclick=()=>runConfirmed(confirm,'Restart Guess the Age? Team membership and manual corrections stay, but all Guess the Age answers and game awards will be removed.',()=>window.gameNightSupabaseActions.restartRound());if($('#newSessionPolished'))$('#newSessionPolished').onclick=()=>runConfirmed(confirm,'Start a fresh session with a new room code? The current event will be kept as history.',()=>window.gameNightSupabaseActions.startNewSession())}
function renderRemoteIBetControl(){
  ...
  if(!group){$('#liveControl').innerHTML=`${roundNavigator()}<div class="host-display-row">${audienceSelector(event)}<button class="btn secondary" id="openAudience">Audience ↗</button></div><div class="control-empty"><h2>I Bet You complete</h2><p>Choose another round above or show the leaderboard.</p><button class="btn primary" id="ibetLeaderboard">Show leaderboard</button></div>`;bindRoundNavigator();bindAudienceSelector();$('#openAudience').onclick=()=>window.open(`audience.html?room=${encodeURIComponent(event.room_code)}`,'gameNightAudience');$('#ibetLeaderboard').onclick=()=>window.gameNightSupabaseActions.setDisplay('leaderboard');return}
  ...
  let controls='';
  if(['waiting','bidding'].includes(group.state))controls=`${group.current_bidder_team_id?`<p class="ibet-committed-bid">Last committed bid: <strong>${mascotEmoji(group.members.find(m=>m.team_id===group.current_bidder_team_id)?.mascot_id)} ${esc(teamName(group,group.current_bidder_team_id))} · ${group.current_bid}</strong></p>`:'<p class="ibet-committed-bid">No bid committed yet. Select the first Team.</p>'}<div class="ibet-bidder-grid">${group.members.map(m=>`<button class="ibet-team-button ${m.team_id===draft.selectedTeamId?'selected':''}" data-team="${m.team_id}" ${draft.pending?'disabled':''}><span>${mascotEmoji(m.mascot_id)}</span>${esc(m.name)}</button>`).join('')}</div><div class="ibet-bid-control"><button class="ibet-number-button" id="ibetMinus" ${draft.pending?'disabled':''}>−</button><strong>${draft.proposedBid}</strong><button class="ibet-number-button" id="ibetPlus" ${draft.pending?'disabled':''}>+</button></div><div class="ibet-bidding-actions"><button class="btn primary" id="ibetCommit" ${draft.pending||!draft.selectedTeamId?'disabled':''}>${draft.pending?'COMMITTING…':'I BET YOU'}</button><button class="btn secondary" id="ibetChallenge" ${draft.pending||!group.current_bidder_team_id||!draft.selectedTeamId||draft.selectedTeamId===group.current_bidder_team_id?'disabled':''}>NAME THEM</button></div>`;
...
function init(){
  ...
  window.addEventListener('game-night-remote-state',e=>{e.detail._hydratedAt=Date.now();window.gameNightRemoteSession=e.detail;document.body.classList.add('hosted-admin');syncRemoteGuessRound();bindEventFields();renderRounds();if(editingRoundId){const current=state.rounds.find(r=>r.id===editingRoundId);if(current)renderDrawer(current)}renderTeams();renderRemoteLeaderboard();renderLiveControl()});
}
init();
