...
function renderRemoteLeaderboard(){const board=remoteSession()?.leaderboard||[];$('#leaderboardTable').innerHTML=`<table><thead><tr><th>Place</th><th>Team</th><th>Points</th></tr></thead><tbody>${board.map((t,i)=>`<tr><td>${i+1}</td><td><strong>${mascotEmoji(t.mascot_id)} ${esc(t.name)}</strong></td><td>${t.points||0}</td></tr>`).join('')||'<tr><td colspan="3">No scores yet</td></tr>'}</tbody></table>`}
